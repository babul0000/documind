...
function ChatWorkspace() {
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-6">
        <div>
          ...
          <p className="text-xs text-muted mt-1">Select an analyzed document from the list below to begin discussing details with the AI assistant.</p>
        </div>

        {loadingDocs ? (
          <div className="rounded-2xl border border-border bg-background/20 p-12 text-center flex flex-col items-center justify-center gap-2">
            <svg className="animate-spin h-5 w-5 text-muted" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" />
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z" />
            </svg>
            <span className="text-xs text-muted font-semibold">Loading documents list...</span>
          </div>
        ) : readyDocs.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-border bg-background/25 p-12 text-center">
            <FileText className="h-10 w-10 text-zinc-700 mx-auto mb-4" />
            <h4 className="text-sm font-bold text-foreground">No ready documents</h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {readyDocs.map((doc) => (
              <Card
                ...
                className="hover:border-zinc-755 hover:bg-muted-bg/20 cursor-pointer transition-all duration-200"
              >
                <CardContent className="p-6 flex items-center justify-between gap-4">
                  <div className="flex items-center gap-4 min-w-0">
                    <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-muted-bg border border-border text-indigo-400">
                      <FileText className="h-5.5 w-5.5" />
                    </div>
                    <div className="min-w-0">
                      <h4 className="text-sm font-bold text-white truncate max-w-[200px]">{doc.originalName}</h4>
                      <p className="text-[10px] text-muted font-semibold mt-0.5">{formatBytes(doc.size)}</p>
                    </div>
                  </div>
                  <ChevronLeft className="h-5 w-5 text-muted rotate-180" />
                </CardContent>
              </Card>
            ))}
          </div>
  ...
  return (
    <div className="flex flex-col xl:flex-row gap-6 h-[calc(100vh-140px)] max-w-[1600px] mx-auto overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 rounded-2xl border border-border bg-background/20 overflow-hidden">
        {/* Document Header details */}
        <div className="px-6 py-4 border-b border-border bg-background/40 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shrink-0">
          <div className="flex items-center gap-3">
            <button
              onClick={() => router.push('/dashboard/chat')}
              className="flex h-8 w-8 items-center justify-center rounded-lg border border-border bg-background text-muted hover:text-white transition-colors"
            >
        ...
        <div className="flex-1 overflow-y-auto p-6 text-sm text-muted leading-relaxed font-medium">
          {loadingDocDetails ? (
            <div className="flex flex-col items-center justify-center h-full gap-2">
              ...
              <span className="text-xs text-muted">Retrieving content...</span>
            </div>
          ) : (
            <>
              {activeTab === 'summary' && (
                <div className="space-y-4 whitespace-pre-line text-foreground">
                  {activeDoc?.summary ? (
                    ...
                    <>
                      ...
                      {activeDoc.keyInfo.keyTopics && activeDoc.keyInfo.keyTopics.length > 0 && (
                        <div className="rounded-xl border border-border bg-background/40 p-5">
                          <h4 className="text-xs font-bold uppercase tracking-wider text-muted mb-3">Key Topics & Keywords</h4>
                          <div className="flex flex-wrap gap-2">
                            {activeDoc.keyInfo.keyTopics.map((topic, i) => (
                              <span key={i} className="rounded-lg bg-muted-bg border border-border px-2.5 py-1 text-xs text-foreground font-semibold">
                                {topic}
                              </span>
                            ))}
                          </div>
                      ...
                      {activeDoc.keyInfo.dates && activeDoc.keyInfo.dates.length > 0 && (
                        <div className="rounded-xl border border-border bg-background/40 p-5">
                          ...
                          <ul className="list-disc list-inside space-y-1.5 text-xs font-semibold text-foreground">
              ...
              {activeTab === 'context' && (
                <div className="space-y-4">
                  <div className="bg-background/60 rounded-xl p-5 border border-border font-mono text-xs text-muted h-[380px] overflow-y-auto whitespace-pre-wrap select-all">
                    {activeDoc?.textContent || 'No readable text content extracted.'}
                  </div>
                  <p className="text-[10px] text-muted font-bold uppercase tracking-wider text-right">Double-click text block to select all</p>
                </div>
              )}
            </>
          )}
...
export default function ChatPage() {
  return (
    <Suspense fallback={
      <div className="flex h-full w-full items-center justify-center p-12 bg-background text-muted">
        <div className="flex flex-col items-center gap-2">
          <svg className="animate-spin h-5 w-5 text-zinc-700" fill="none" viewBox="0 0 24 24">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" />
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z" />
          </svg>
          <span className="text-xs text-muted font-semibold uppercase tracking-wider">Mounting workspace...</span>
        </div>
      </div>
    }>
      <ChatWorkspace />
